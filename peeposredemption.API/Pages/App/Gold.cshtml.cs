...
using peeposredemption.Application.Features.Shop.Queries;
...
public class GoldModel : PageModel
{
    ...
    public GoldModel(IMediator mediator, IUnitOfWork uow) { _mediator = mediator; _uow = uow; }

    public ServerListViewModel ServerList { get; set; } = new();
    public GoldSubscriptionDto? Subscription { get; set; }
    public bool ShowActivatedBanner { get; set; }

    public async Task<IActionResult> OnGetAsync(bool activated = false)
    {
        ...
        Subscription = await _mediator.Send(new GetGoldSubscriptionQuery(userId));
        ShowActivatedBanner = activated && Subscription?.IsActive == true;
